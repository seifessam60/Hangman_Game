...
document.addEventListener('click',(e)=>{
    ...
    if (target.className ==='letter-box') {
        ...
        if (gameStatus===false) {
            gameAttempts++;
            document.querySelector('.hangman-draw')?.classList.add(`wrong-${gameAttempts}`);
            const fail = document.getElementById('fail') as  HTMLAudioElement
            fail.play();
            if (gameAttempts===8) {
            ...
            const success = document.getElementById('success') as  HTMLAudioElement
            success.play();
        }
    }
})
